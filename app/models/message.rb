...
class Message < ApplicationRecord
  ...
  after_create_commit :auto_create_kanban_item_for_incoming
  ...
  def can_delete_message?
    inbox.allow_agent_to_delete_message
  end
  ...
  def auto_create_kanban_item_for_incoming
    return unless incoming?
    Rails.logger.info("[Kanban] AutoCreateItemJob disparado para conversation_id=#{conversation.id}, message_id=#{id}")
    AutoCreateItemJob.perform_later(conversation.id)
  end
