module Enterprise::Account::PlanUsageAndLimits
  ...
  def validate_limit_keys
    ...
    # Clean invalid values: remove empty strings, nil, and non-numeric values
    valid_keys = %w[inboxes agents captain_responses captain_documents]
    cleaned_limits = {}
    self[:limits].each do |key, value|
      next unless valid_keys.include?(key.to_s)
      next if value.blank? || value.to_s.strip.empty?

      numeric_value = value.to_f
      cleaned_limits[key.to_s] = numeric_value if numeric_value.finite?
    end

    self[:limits] = cleaned_limits
